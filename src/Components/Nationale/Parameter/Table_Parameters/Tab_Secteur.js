...
function Tab_Secteur() {
  ...
  const Initialiser = (_id) => {
    axios
      .put(`${lien_update}/updatesecteur/${_id}`, config)
      .then((response) => {})
  }
      ...
      renderCell: (params) => {
        return (
          <>
            ...
            >
              <Edit onClick={(e) => modifier(e, params.row)} color="default" />
            </Fab>

            <span
              style={{ cursor: 'pointer', margin: '10px', color: 'green' }}
              onClick={() => Initialiser(params.row._id)}
            >
              Initiale
            </span>
          </>
  ...
  return (
    <div>
      ...
      <Popup
        ...
        title="Enregistrer la province"
      >
        <FormSecteur reload={read_Secteur} />
      </Popup>
      <Popup
        openPopup={openPopupModifier}
        setOpenPopup={setOpenPopupModifier}
