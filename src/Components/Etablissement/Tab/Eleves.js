...
function EleveTable({dataLog}) {
    ...
    const fechAgent = async (e) => {
        if (dataLog) {
            const response = await axios.get(`${lien_read}/readEleve/${
                data[0].codeEtablissement
            }`, config)
    ...
    const columns = [
            ...
            renderCell: (params) => {
                return (
                    <Avatar sx={
                        ...
                        {
                        !isEmpty(params.row.filename) ? <img src={
                                `${lien_image_eleve}/${
                                    params.row.filename
                                }`
                            }
                            alt={
                                params.row.agent[0].filename
        ...
        },
        {
            ...
            renderCell: (params) => {
                return <>{
                    new Date(params.row.date_Naissance).toLocaleDateString()
                }</>
            }
        }, {
            field: 'code_eleve',
