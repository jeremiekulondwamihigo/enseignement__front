  ...
  const fechAgent = async (e) => {
    if (dataLog) {
      const response = await axios.get(
        `${lien_read}/allAgentEtablissement/${dataLog.data[0].codeEtablissement}`,
        config,
      )
  ...
  const columns = [
    {
      ...
      renderCell: (params) => {
        return (
          <Avatar>
            <img
              src={`${lien_image_admin}/${params.row.agent[0].filename}`}
              alt={params.row._id}
            />
          </Avatar>
        )
        ...
        {dataLog ? (
          <>
            ...
            <main>
              ...
              <div
                style={{ position: 'absolute', top: '30px', right: '40rem' }}
              >
